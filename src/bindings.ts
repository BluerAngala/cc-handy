...
async fetchLatestMarket(sort: string | null, limit: number | null) : Promise<Result<ModelInfo[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("fetch_latest_market", { sort, limit }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type ModelFile = { url: string; target_name: string }
export type ModelInfo = { id: string; name: string; description: string; filename: string; url: string | null; files: ModelFile[] | null; sha256: string | null; size_mb: number; is_downloaded: boolean; is_downloading: boolean; partial_size: number; is_directory: boolean; engine_type: EngineType; accuracy_score: number; speed_score: number; supports_translation: boolean; is_recommended: boolean; supported_languages: string[]; supports_language_selection: boolean; is_custom: boolean; downloads: number | null; likes: number | null; created_at: string | null; trending_score: number | null }
